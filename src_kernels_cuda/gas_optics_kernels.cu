...
__device__ __forceinline__ void interpolate2D_byflav_kernel(const Float *__restrict__ fminor,
...
__device__ void interpolate3D_byflav_kernel(
...
__global__ void reorder12x21_kernel(
...
__global__ void reorder123x321_kernel(
...
__global__ void zero_array_kernel(
    const int ni, const int nj, const int nk,
...
__global__ void Planck_source_kernel(
...
__global__ void gas_optical_depths_major_kernel(
    const int ncol, const int nlay, const int nband, const int ngpt,
    const int nflav, const int neta, const int npres, const int ntemp,
            ...
            if(tropo[idx_collay]==idx_tropo)
            {
                for(int imnr=0;imnr<nminor;++imnr)
                {
                    ...
                    if (minor_scales_with_density[imnr])
                    {
                        ...
                        if (idx_minor_scaling[imnr] > 0)
                        {
                            int idx_collaywv = icol + ilay*ncol + idx_h2o*ncl;
                            Float vmr_fact = Float(1.) / col_gas[idx_collay];
                            Float dry_fact = Float(1.) / (Float(1.) + col_gas[idx_collaywv] * vmr_fact);
...
/*test 2
#if use_shared_tau==0
template<int block_size_x, int block_size_y, int block_size_z> __host__
void gas_optical_depths_minor_kernel(
        const int ncol, const int nlay, const int ngpt,
        const int ngas, const int nflav, const int ntemp, const int neta,
        const int nminor,
        ...
        int nblockiddz)
{
    ...
    int ilay=0;
    int icol=0;
    Float scalings[block_size_z*nblockiddz][block_size_y*42];
for(blockiddx=0;blockiddx<1;blockiddx++){
for(blockiddy=0;blockiddy<42;blockiddy++){
for(blockiddz=0;blockiddz<nblockiddz;blockiddz++){
for(threadiddx=0;threadiddx<block_size_x;threadiddx++){
for(threadiddy=0;threadiddy<block_size_y;threadiddy++){
for(threadiddz=0;threadiddz<block_size_z;threadiddz++){
    ilay = blockiddy * block_size_y + threadiddy;
    icol = blockiddz * block_size_z + threadiddz;
        ...
        if (tropo[idx_collay] == idx_tropo)
        {
            for (int imnr=0; imnr<nminor; ++imnr)
            {
                ...
                if (threadiddx == 0)
                {
                    ...
                    scalings[threadiddz*blockiddz][threadiddy*blockiddy] = scaling;
                }

                scaling = scalings[threadiddz*blockiddz][threadiddy*blockiddy];
                ...
                if constexpr (block_size_x == 16)
                {
                    if (threadiddx < band_gpt)
                    {
                        ...
                        int idx_out = icol + ilay*ncol + (igpt+gpt_start)*ncol*nlay;
                        tau[idx_out] += ltau_minor * scaling;
                    }
                }
                else
                {
                    for (int igpt=threadiddx; igpt<band_gpt; igpt+=block_size_x)
                    {
                        Float ltau_minor = kfminor[0] * kin[(kjtemp-1) + (j0-1)*ntemp + (igpt+gpt_offset)*ntemp*neta] +
                                        kfminor[1] * kin[(kjtemp-1) +  j0   *ntemp + (igpt+gpt_offset)*ntemp*neta] +
                                        kfminor[2] * kin[kjtemp     + (j1-1)*ntemp + (igpt+gpt_offset)*ntemp*neta] +
...
#endif
*/
// TEST 3 quasi funzionante

#if use_shared_tau == 0
template <int block_size_x, int block_size_y, int block_size_z>
__host__ void gas_optical_depths_minor_kernel(
    const int ncol, const int nlay, const int ngpt,
    const int ngas, const int nflav, const int ntemp, const int neta,
    const int nminor,
    const int nminork,
    ...
    const Bool *__restrict__ tropo,
    Float *__restrict__ tau,
    Float *__restrict__ tauminor)
{
    int nblockiddz = 0;
    if constexpr (block_size_z == 16)
    {
        nblockiddz = 8;
    }
    else
        nblockiddz = 4;
    //per parallelizzare rimuovere commenti di #pragma
    Float scalings[ncol * 2][nlay * 2]; //to comment if parallel
    int ilay = 0;
    int icol = 0;
    int igpt = 0;
    ...
    for (int imnr = 0; imnr < nminor; ++imnr)
    {
        // Float scalings[ncol * 2][nlay * 2]; //togliere il commento per parallelizzare
        // #pragma omp parallel for collapse(2)
        for (ilay = 0; ilay < 43; ilay++)
        {
            for (icol = 0; icol < block_size_z * (nblockiddz + 1); icol++)
            {
                if ((icol < ncol) && (ilay < nlay))
                {
                    Float scaling = Float(0.);
        ...
        for (ilay = 0; ilay < 43; ilay++)
        {
            for (icol = 0; icol < block_size_z * (nblockiddz + 1); icol++)
            {
                for (int threadx = 0; threadx < block_size_x; threadx++)
                {
                    if ((icol < ncol) && (ilay < nlay))
                    {
                        const int idx_collay = icol + ilay * ncol;
                        if (tropo[idx_collay] == idx_tropo)
                        {
                            ...
                            const int band_gpt = gpt_end - gpt_start;
                            const int gpt_offset = kminor_start[imnr] - 1;
                            for (int igpt = threadx; igpt < band_gpt; igpt += block_size_x)
                            {
                                Float ltau_minor = kfminor[0] * kin[(kjtemp - 1) + (j0 - 1) * ntemp + (igpt + gpt_offset) * ntemp * neta] +
                                                   kfminor[1] * kin[(kjtemp - 1) + j0 * ntemp + (igpt + gpt_offset) * ntemp * neta] +
                                                   kfminor[2] * kin[kjtemp + (j1 - 1) * ntemp + (igpt + gpt_offset) * ntemp * neta] +
...
/*
#if use_shared_tau == 0
template<int block_size_x, int block_size_y, int block_size_z, int nblockx,int nblocky,int nblockz> __host__
void gas_optical_depths_minor_kernel(
        const int ncol, const int nlay, const int ngpt,
        const int ngas, const int nflav, const int ntemp, const int neta,
        const int nminor,
        ...
        Float* __restrict__ tau_minor)
{
    ...
    for(int blockx=0;blockx<nblockx;blockx++)
    {
        ...
        {
            for(int blockz=0;blockz<nblockz;blockz++)
            {
                ...
                for(int imnr=0;imnr<nminor;++imnr)
                {
                    for(int threadx=0;threadx<block_size_x;threadx++)
                    {
                        for(int thready=0;thready<block_size_y;thready++)
                        {
                            for(int threadz=0;threadz<block_size_z;threadz++)
                            {
                                const int ilay = blocky * block_size_y + thready;
                                const int icol = blockz * block_size_z + threadz;
                                ...
                                if ( (icol < ncol) && (ilay < nlay) )
                                {
                                    ...
                                    scalings[threadz][thready] = scaling;
                                    }
                                }
                            }
                        }
                        ...
                        for(int thready=0;thready<block_size_y;thready++)
                        {
                            for(int threadz=0;threadz<block_size_z;threadz++)
                            {
                                ...
                                if ( (icol < ncol) && (ilay < nlay) )
                                {
                                    ...
                                    if (tropo[idx_collay] == idx_tropo)
                                    {
                                        Float scaling = scalings[threadz][thready];
...
__global__ void compute_tau_rayleigh_kernel(
...
__global__ void combine_abs_and_rayleigh_kernel(
